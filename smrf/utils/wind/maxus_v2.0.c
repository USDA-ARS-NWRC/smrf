...
float deg_direction;
{
	...
	{
		while (elev >= 0 && DISTANCE < (float) dmax)
		{
			tempi = endi - 1;
...
{
	int p, *IntCell, current_elevation, elev, i, j, N, found;
...
main (int argc, char *argv[])
{
	...
	float accum_hor, index, hord, w, increment;
	Grid Hor;
	char output_file[50];
	char dem_file[50];
	...
	for (row = 0; row < Adjdem->NumRows; row++)  {
		for (col = 0; col < Adjdem->NumCols; col++)  {
			if (row == 0 || row == Adjdem->NumRows - 1 || \
				...
				Put_Cell_Value(row, col, Adjdem, *IntCell, FloatValue);
			}
		}
	}
