...
class TestGradient(SMRFTestCase):
    ...
    def test_gradient_d4_funcs(self):
        """ Small tests for the gradient_d4 """

        dx = 1
        dy = 1

        # with dx and dy equal to 1, the cardinal direction
        # slope values will be np.pi/4 as one of the differences
        # will be zero

        # test west slope and aspect
        dem = np.tile(range(10), (10, 1))
        py_slope, asp = gradient.gradient_d4(dem, dx, dy)
        ...
        # with dx and dy equal to 1, the slope of the 45 degree
        ...
        self.assertTrue(np.all(py_slope == slope_val))
    ...
    def test_gradient_d8_funcs(self):
        """ Small tests for the gradient_d8 """
        ...
        # test west slope and aspect
        dem = np.tile(range(10), (10, 1))
        py_slope, asp = gradient.gradient_d8(dem, dx, dy)
        ipw_a = gradient.aspect_to_ipw_radians(asp)
